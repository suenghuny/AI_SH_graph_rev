...
def train(agent, env, e, t, train_start, epsilon, min_epsilon, anneal_epsilon, initializer, output_dir, vdn):
    agent_blue = Policy(env, rule='rule2', temperatures=temperature)
    ...
    while not done:
        if env.now % (decision_timestep) <= 0.00001:
            ...
            action_blue = agent_yellow.get_action(avail_action_blue, target_distance_blue, air_alert_yellow)
...
if __name__ == "__main__":
    ...
    for e in range(num_iteration):
        ...
        if e % 10 == 0:
            import os
            import pandas as pd
            output_dir = "../output_dir_susceptibility_heuristic/"
