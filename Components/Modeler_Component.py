...
class Environment:
    ...
    def get_target_availability(self, friendlies_fixed_list, avail_action_friendly_model, enemies_fixed_list, flying_ssms_enemies):
        ...
        for ship in friendlies_fixed_list:
            ...
            if distance_list==list():
                ...
                distance_list.insert(0, np.max(distance_list))
    ...
    def get_missile_node_feature(self):
        ...
        for ship in self.friendlies_fixed_list:
            ...
            #             [px / enemy.attack_range, py / enemy.attack_range, vx / enemy.speed_m, vy / enemy.speed_m, ax,
            #              ay,0,1])
    ...
    def step(self, action_blue, action_yellow, rl = True, pass_transition = False):
        ...
        if pass_transition == False:
            ...
            reward = 50 * (missile_destroyed_cal - self.last_destroyed_missile) + \
                     500 * (enemy_destroyed_cal - self.last_destroyed_enemy) + \
                     -1000 * (ship_destroyed_cal - self.last_destroyed_ship)
            reward = reward / 100
