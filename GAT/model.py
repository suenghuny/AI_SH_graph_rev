...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout, alpha, nheads, teleport_probability,  batch_size, mode = 'observation'):
        ...
        if mode == 'communication':
            self.attentions1 = [GraphAttentionLayer(nfeat, nhid, dropout=dropout, alpha=alpha, concat=True, teleport_probability=self.teleport_probability, batch_size = batch_size).to(device)
                                for _ in range(nheads)]
            for i, attention in enumerate(self.attentions1):
                self.add_module('attention_{}'.format(i), attention)
            self.attentions2 = [GraphAttentionLayer(nhid * nheads, nhid, dropout=dropout, alpha=alpha, concat=True, teleport_probability=self.teleport_probability, batch_size = batch_size).to(device) for _ in
                range(nheads)]

            for i, attention in enumerate(self.attentions2):
                self.add_module('attention_{}'.format(i), attention)

            self.out_att = GraphAttentionLayer(nhid * nheads, nclass, dropout=dropout, alpha=alpha, concat=False, teleport_probability=self.teleport_probability, batch_size = batch_size).to(
                device)
